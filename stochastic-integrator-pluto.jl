...
md"""Differential equation for each of the phase space components"""
...
=
\boldsymbol{\nabla} \cdot \boldsymbol{\kappa}_\perp \cdot \boldsymbol{\nabla} f
- \left(v\mu \hat{\mathbf{b}} + \mathbf{V} + \mathbf{V}_d\right) \cdot \boldsymbol{\nabla} f
+ \frac{\partial}{\partial \mu} D_{\mu\mu} \frac{\partial f}{\partial \mu}
...
```math
\begin{align}
d\mathbf{x}(t) &= \sqrt{2 \boldsymbol{\kappa}_\perp} \cdot d\mathbf{w}(t) + \left(\boldsymbol{\nabla} \cdot \boldsymbol{\kappa}_\perp + v\mu\hat{\mathbf{b}} + \mathbf{V} + \mathbf{V}_d\right) dt \\
d\mu(t) &= \sqrt{2 \max(D_{\mu\mu},0)} \, dw(t) + \left(\frac{\partial D_{\mu\mu}}{\partial \mu} + \frac{d\mu}{dt}\right) dt \\
dp(t) &= \frac{dp}{dt} \, dt
\end{align}
```
"""
...
md""" $w(t)$ is a normally distributed Wiener noise process."""

# ╔═╡ 3732a5b2-5810-48d5-953d-7ae4c906ff47
md"""
Perpendicular diffusion coefficient:

$\kappa_\perp = \frac{v}{2} D_\text{FLRW}$

Assume $\kappa_\perp$ is a diagonal tensor.

$\boldsymbol{\kappa}_\perp = \begin{bmatrix} \kappa_\perp & \cdot & \cdot \\ \cdot & \kappa_\perp & \cdot \\ \cdot & \cdot & \kappa_\perp \end{bmatrix}$

Identity: $\kappa_\perp = \frac{1}{3} \operatorname{tr}(\boldsymbol{\kappa}_\perp)$

Divergence of the tensor is the gradient of the scalar:

$\boldsymbol{\nabla} \kappa_\perp = \boldsymbol{\nabla} \cdot \boldsymbol{\kappa}_\perp = \tfrac{1}{2} \boldsymbol{\nabla} (v D_\text{FLRW}) = \tfrac{1}{2} \left(\boldsymbol{\nabla} v \cdot D_\text{FLRW} + v \cdot \boldsymbol{\nabla} D_\text{FLRW} \right)$

Question: can the square root of the tensor be taken as the square root of the scalar?
```math
\sqrt{2 \boldsymbol{\kappa}_\perp} \cdot d\mathbf{w}(t) ~\overset{?}{=}~ \sqrt{2 \kappa_\perp} \, d\mathbf{w}(t)
```

Answer: Yes? If it's a multiple of the identity tensor?

```math
\sqrt{\lambda \mathbf{1}} = \sqrt{\lambda} \sqrt{\mathbf{1}} = \sqrt{\lambda} \mathbf{1}
```
"""
...
$\begin{align*}
\mathbf{u}(t) &= \begin{bmatrix} \mathbf{x}(t) \\ \mu(t) \\ p(t) \end{bmatrix} \\[2ex]
d\mathbf{u} &= f\,dt + g\,dw \\[1.5ex]
d\mathbf{u} &= \begin{bmatrix} \boldsymbol{\nabla} \cdot \boldsymbol{\kappa}_\perp + v\mu\hat{\mathbf{b}} + \mathbf{V} + \mathbf{V}_d \\[0.5ex] \displaystyle \frac{\partial D_{\mu\mu}}{\partial \mu} + \frac{d\mu}{dt} \\[0.5ex] \displaystyle \frac{dp}{dt} \end{bmatrix} dt + \begin{bmatrix} \sqrt{2\kappa_\perp} \\[3.3ex]  \sqrt{2\max(D_{\mu\mu},0)} \\[3.3ex] 0 \end{bmatrix} dw
\end{align*}$
"""
...
manifest_format = "2.0"
project_hash = "eb335051cdc05d0446f10ac1524852e602a281d0"
